...
//
static double print_scale_data(unsigned char*);
...
// should terminate.
//
static double print_scale_data(unsigned char* dat) 
{
    // 
        ...
        for (i = 0; i < NSCALES; i++) 
        {
            if(desc.idVendor  == scales[i][0] && 
               desc.idProduct == scales[i][1]) 
               {
                  ...
                  r = libusb_get_string_descriptor_ascii(hand, desc.iManufacturer, string, 256);
                  fprintf(stderr,
                          "Manufacturer: %s\n", string);
                  libusb_close(hand);
#endif
